...
-- Retornar produtor com valor maior que 50 reais
select * from tb_produto where preco > 50.00;

-- Retornar produtos com valor entre 3 e 60 reais
select * from tb_produto where preco between 3.00 and 60.00;

-- Retornar produtos com a CI em qualquer posição
select * from tb_produto where nome like "%CI%";

-- Usando inner join entre tabela categoria e produto mostra nome, marca, preco, descricao (categoria primeira tabela) e id_categoria
select tb_produto.id nome, marca, preco, tb_categoria.descricao as descricao, tb_categoria.id as id_categoria
from tb_produto
Inner join tb_categoria on tb_categoria.id = tb_produto.categoria_id;

-- Traz produtos de uma categoria específica
